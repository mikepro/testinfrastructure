...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  #   # You may also specify custom JSON attributes:
     chef.json = {'rabbitmq'=>{
         'enabled_plugins'=>['rabbitmq_consistent_hash_exchange'],
         'virtualhosts'=>['/','operations','logs'],
         'enabled_users'=>[{:name=>'user', :password=>'user1234',:tag=>'administrator', :rights=>
         [{:vhost=>'/', :conf=>'.*', :write=>'.*',:read=>'.*'},
          {:vhost=>'operations', :conf=>'.*', :write=>'.*',:read=>'.*'},
          {:vhost=>'logs', :conf=>'.*', :write=>'.*',:read=>'.*'},
         ]
     }]
     } }
   end
